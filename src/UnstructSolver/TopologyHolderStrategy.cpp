...
#include <iostream>
#include <cmath>
void TopologyHolderStrategy::initializeStrategy(UnstructTopologyHolder *hder)
{
    d_hder = hder;
    d_dim = d_hder->d_dim;
    ...
    d_iblank_cell = new int*[d_nmesh];
    for(int i = 0;i<d_nmesh;i++)
    {
        d_iblank_cell[i] = new int[d_hder->nCells(i)];
    }
...
void TopologyHolderStrategy::initializePrimData(double** W)
{
    const double Gamma = 1.4;
    double **U = getU();
    for(int i = 0;i<d_nmesh;i++)
...
void TopologyHolderStrategy::set_fs_variable2(double Ma, double AOA, double density, double pressure, double Re)
{
    ...
    fs_mu = 1.458e-6*pow(fs_Temperature,1.5)/(fs_Temperature+110.5);
