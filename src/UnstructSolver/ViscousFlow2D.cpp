...
void ViscousFlow2D::set_fs_variable(double Ma, double AOA, double density, double pressure, double eigenLen)
{
    // Ma is 0.8
    fs_mach = Ma;
    // AOA is 1.25
...
void ViscousFlow2D::update_nondim_variable()
{
    // u v
...
void ViscousFlow2D::test_unwantedSweep(int** UnwantedInd,int* nUnwantedInd)
{
...
void ViscousFlow2D::test_communication()
{
...
void ViscousFlow2D::test_partialcomm()
{
    testing_flag = new int *[d_hder->d_nmesh];
    for (int i = 0; i < d_hder->d_nmesh; i++)
    {
...
void ViscousFlow2D::writeTiogaFormat(const std::string &filename, int proc, int meshTag, int *icelldata)
{
...
{
    //This should be packed into one strategy.
...
void ViscousFlow2D::updateEdgeLeftRightValues()
{
    // if (cur_proc == 0)
...
void ViscousFlow2D::AllCellCommunication(double** value)
{
    // if (cur_proc == 0)
...
void ViscousFlow2D::writeCellData(const std::string &filename, int proc, int meshTag, double **dcelldata)
{
...
void ViscousFlow2D::writeUnacceptableCell(const std::string &filename, int proc, int meshTag, int **dcelldata)
{
    std::string local_filename = filename + std::to_string(meshTag) + std::to_string(proc);
    std::ofstream fout;
    fout.open(local_filename);
...
void ViscousFlow2D::RemoteCellCommunication(double** value)
{
    int temp_int = 0;
    for (int i = 0;i < d_hder->d_nmesh; i++)
    {
...
void ViscousFlow2D::NearCellCommunication(double** value)
{
    int temp_int = 0;
    for (int i = 0;i < d_hder->d_nmesh; i++)
    {
