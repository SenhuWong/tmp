...
void parallelComm::sendRecvPacketsAll(PACKET* sndPack, PACKET* rcvPack)
{
    ...
    MPI_Status* status;
    //
    sint = new int[numprocs];
    sreal = new int[numprocs];
    rint = new int[numprocs];
    rreal = new int[numprocs];
    request = new MPI_Request[4 * numprocs];
    status = new MPI_Status[4 * numprocs];
    ...
    for (i = 0; i < numprocs; i++)
    {
        if (rcvPack[i].nints > 0) {
            tag = 1;
            rcvPack[i].intData = new int[rcvPack[i].nints];
        ...
        if (rcvPack[i].nreals > 0) {
            tag = 2;
            rcvPack[i].realData = new REAL[rcvPack[i].nreals];
...
void parallelComm::sendRecvPackets(PACKET* sndPack, PACKET* rcvPack)
{
    ...
    MPI_Status* status;
    //
    scount = new int[2 * nsend];
    rcount = new int[2 * nrecv];
    request = new MPI_Request[2 * (nsend + nrecv)];
    status = new MPI_Status[2 * (nsend + nrecv)];
