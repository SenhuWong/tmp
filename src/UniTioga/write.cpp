...
void MeshBlock::writeGridFile2(const std::string& filename, double* nodeInfo, int nvar)
{
    std::string localFilename = filename + std::to_string(meshtag) + std::to_string(myid) + ".dat";
    std::ofstream fout;
...
void MeshBlock::writeCellFile2(const std::string& filename,int nvar, double* cellInfo)
{
    std::string localFilename = filename + std::to_string(meshtag) + std::to_string(myid) + ".dat";
    std::ofstream fout;
    ...
    {
        fout << d_dim + i + 1<< "=CELLCENTERED,";
    }
    fout << d_dim + nvar+ 1 << "=CELLCENTERED)\n";
    for (int j = 0; j < d_dim; j++)
    {
        for (int i = 0; i < nnodes; i++)
        {
