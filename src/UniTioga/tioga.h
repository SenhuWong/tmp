...
namespace TIOGA
{
    ...
    public:
        ...
        CartBlock *cb = NULL;
        int nmesh = 0;
        HOLEMAP *holeMap = NULL;
        MPI_Comm scomm;
        parallelComm *pc = NULL;
        parallelComm *pc_cart = NULL;
        int isym = 3;
        int ierr = 0;
        int myid = -1;
        int numprocs = -1;
    ...
    public:
        ...
        void exchangeDonors(bool reduce_fringe);
        ...
        void outputHoleMap(const std::string &filename);
        ...
        */
        ...
        void register_amr_global_data(int nf, int qstride, double *qnodein, int *idata, double *rdata, int ngridsin, int qnodesize);
        void set_amr_patch_count(int npatchesin);
        void register_amr_local_data(int ipatch, int global_id, int *iblank, double *q);
        ...
        void outPutQuery(const std::string &filename);
        void outPutCartQuery(const std::string &filename);
        //void outPutGoodQuery(const std::string &filename);
        void test_from_search(double *xsearches, int nsearches);
        bool d_uses_distancing = false;
        void set_distanced(bool distancing);
        void find_distances()
        {
            for (int ib = 0; ib < nblocks; ib++)
            {
        ...
        void output_distance()
        {
            ...
            {
                auto &mb = mblocks[ib];
                mb->write_distance_files("NodeDistance");
            }
        }
    };
