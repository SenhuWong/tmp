...
Euler::Euler(
    ...
    std::shared_ptr<geom::CartesianGridGeometry> grid_geom) : algs::HyperbolicPatchStrategy(),
                                                              ...
                                                              d_iblank(new pdat::CellVariable<int>(dim, "iblank")),
                                                              ...
                                                              d_number_of_intervals(0)
{
    ...
    if (!t_init)
    {
        t_init = tbox::TimerManager::getManager()->getTimer("apps::Euler::initializeDataOnPatch()");
        t_compute_dt = tbox::TimerManager::getManager()->getTimer("apps::Euler::computeStableDtOnPatch()");
        t_compute_fluxes = tbox::TimerManager::getManager()->getTimer("apps::Euler::computeFluxesOnPatch()");
        t_conservdiff = tbox::TimerManager::getManager()->getTimer("apps::Euler::conservativeDifferenceOnPatch()");
        t_setphysbcs = tbox::TimerManager::getManager()->getTimer("apps::Euler::setPhysicalBoundaryConditions()");
        t_taggradient = tbox::TimerManager::getManager()->getTimer("apps::Euler::tagGradientDetectorCells()");
    }
    ...
    SAMRAI_F77_FUNC(stufprobc, STUFPROBC)
...
{
    // We don't make iblank related to HyperbolicLevelIntegrator
    // It is specific to the interpolation that we want to use
    hier::VariableDatabase *variable_db = SAMRAI::hier::VariableDatabase::getDatabase();
    hier::IntVector no_ghost(d_dim, 0);
    const int ibl_flag = variable_db->registerVariableAndContext(
        d_iblank,
        d_plot_context,
        no_ghost);
    ...
    integrator->registerVariable(d_flux, d_fluxghosts,
                                 ...
                                 "CONSERVATIVE_COARSEN",
                                 "NO_REFINE");
    //With these three, the interpStrategy could easily recompute the toZipData for TIOGA
    d_prim_vars.push_back(d_density);
    d_prim_vars.push_back(d_velocity);
    d_prim_vars.push_back(d_pressure);
    ...
    const bool initial_time)
{
    ...
    if (initial_time)
    {
        ...
        if (d_data_problem == "SPHERE")
        {
            ...
            if (d_dim == tbox::Dimension(2))
            {
                SAMRAI_F77_FUNC(eulerinitsphere2d, EULERINITSPHERE2d)
                (d_data_problem_int,
                 dx, xlo, xhi,
                 ifirst(0), ilast(0),
                 ifirst(1), ilast(1),
            ...
            {
                SAMRAI_F77_FUNC(eulerinitsphere3d, EULERINITSPHERE3d)
                (d_data_problem_int,
                 dx, xlo, xhi,
                 ifirst(0), ilast(0),
                 ifirst(1), ilast(1),
        ...
        else
        {
            ...
            if (d_dim == tbox::Dimension(2))
            {
                SAMRAI_F77_FUNC(eulerinit2d, EULERINIT2D)
                (d_data_problem_int,
                 dx, xlo, xhi,
                 ifirst(0), ilast(0),
                 ifirst(1), ilast(1),
            ...
            {
                SAMRAI_F77_FUNC(eulerinit3d, EULERINIT3D)
                (d_data_problem_int,
                 dx, xlo, xhi,
                 ifirst(0), ilast(0),
                 ifirst(1), ilast(1),
    ...
    //Modified here:
    //%TODO::Allocate Space with component selector.
    d_oversetStrategy->tagOverBoundaryCrossCells(patch,data_time);
    t_init->stop();
}
    ...
    const double dt_time)
{
    ...
    if (d_dim == tbox::Dimension(2))
    {
        SAMRAI_F77_FUNC(stabledt2d, STABLEDT2D)
        (dx,
         ifirst(0), ilast(0),
         ifirst(1), ilast(1),
         ghost_cells(0),
    ...
    {
        SAMRAI_F77_FUNC(stabledt3d, STABLEDT3D)
        (dx,
         ifirst(0), ilast(0),
         ifirst(1), ilast(1),
         ifirst(2), ilast(2),
    ...
    const double dt)
{
    ...
    if (d_dim == tbox::Dimension(2))
    {
         ...
         */
        SAMRAI_F77_FUNC(inittraceflux2d, INITTRACEFLUX2D)
        (ifirst(0), ilast(0),
         ifirst(1), ilast(1),
         density->getPointer(),
         velocity->getPointer(),
        ...
        if (d_godunov_order > 1)
        {
             ...
             */
            SAMRAI_F77_FUNC(computesound2d, COMPUTESOUND2D)
            (ifirst(0), ilast(0),
             ifirst(1), ilast(1),
             d_gamma,
             density->getPointer(),
             ...
             *  Output: w^L, w^R
             */
            SAMRAI_F77_FUNC(chartracing2d0, CHARTRACING2D0)
            (dt,
             ifirst(0), ilast(0),
             ifirst(1), ilast(1),
            ...
            SAMRAI_F77_FUNC(chartracing2d1, CHARTRACING2D1)
            (dt,
             ifirst(0), ilast(0),
             ifirst(1), ilast(1),
         ...
         *  Output: F (flux)
         */
        SAMRAI_F77_FUNC(fluxcalculation2d, FLUXCALCULATION2D)
        (dt, 1, 0, dx,
    ...
    const double dt)
{
    ...
    if (d_godunov_order > 1)
    {
         ...
         * Compute local sound speed in each computational cell.
         */
        SAMRAI_F77_FUNC(computesound3d, COMPUTESOUND3D)
        (ifirst(0), ilast(0),
         ifirst(1), ilast(1),
         ifirst(2), ilast(2),
         d_gamma,
         ...
         *  Output: w^L, w^R
         */
        SAMRAI_F77_FUNC(chartracing3d0, CHARTRACING3D0)
        (dt,
         ifirst(0), ilast(0),
         ifirst(1), ilast(1),
         ifirst(2), ilast(2),
        ...
        SAMRAI_F77_FUNC(chartracing3d1, CHARTRACING3D1)
        (dt,
         ifirst(0), ilast(0),
         ifirst(1), ilast(1),
         ifirst(2), ilast(2),
        ...
        SAMRAI_F77_FUNC(chartracing3d2, CHARTRACING3D2)
        (dt,
         ifirst(0), ilast(0),
         ifirst(1), ilast(1),
         ifirst(2), ilast(2),
    ...
    SAMRAI_F77_FUNC(fluxcalculation3d, FLUXCALCULATION3D)
    (dt, 1, 0, 0, dx,
     ...
     */
    SAMRAI_F77_FUNC(fluxcorrec2d, FLUXCORREC2D)
    (dt,
     ifirst(0), ilast(0), ifirst(1), ilast(1), ifirst(2), ilast(2),
     dx, d_gamma, 1,
     density->getPointer(),
     ...
     *  Output: w^L, w^R (traced_left/right)
     */
    SAMRAI_F77_FUNC(fluxcorrec3d, FLUXCORREC3D)
    (dt,
     ifirst(0), ilast(0), ifirst(1), ilast(1), ifirst(2), ilast(2),
     dx, d_gamma,
     density->getPointer(),
    ...
    const double dt)
{
    ...
    for (int idir = 0; idir < d_dim.getValue(); ++idir)
    {
         ...
         *    Output:  third_state
         */
        SAMRAI_F77_FUNC(onethirdstate, ONETHIRDSTATE)
        (dt, dx, idir,
         ifirst(0), ilast(0), ifirst(1), ilast(1), ifirst(2), ilast(2),
         d_gamma,
         density->getPointer(),
         ...
         */
        SAMRAI_F77_FUNC(fluxthird, FLUXTHIRD)
        (dt, dx, idir,
         ifirst(0), ilast(0), ifirst(1), ilast(1), ifirst(2), ilast(2),
         d_gamma,
         d_riemann_solve_int,
         ...
         *    Output:  w^L, w^R (traced_left/right)
         */
        SAMRAI_F77_FUNC(fluxcorrecjt, FLUXCORRECJT)
        (dt, dx, idir,
         ifirst(0), ilast(0), ifirst(1), ilast(1), ifirst(2), ilast(2),
         d_gamma,
         density->getPointer(),
    ...
    bool at_syncronization)
{
    ...
    if (d_dim == tbox::Dimension(2))
    {
        SAMRAI_F77_FUNC(consdiff2d, CONSDIFF2D)
        (ifirst(0), ilast(0), ifirst(1), ilast(1),
         dx,
         flux->getPointer(0),
         flux->getPointer(1),
    ...
    {
        SAMRAI_F77_FUNC(consdiff3d, CONSDIFF3D)
        (ifirst(0), ilast(0), ifirst(1), ilast(1),
         ifirst(2), ilast(2), dx,
         flux->getPointer(0),
         flux->getPointer(1),
    ...
    const hier::IntVector &ratio)
{
    ...
    double *diff2 = d_dim ==
                            tbox::Dimension(3)
                        ? new double[coarse_box.numberCells(2) + 1]
                        : 0;
    ...
    if (d_dim == tbox::Dimension(2))
    {
        ...
        SAMRAI_F77_FUNC(conservlinint2d, CONSERVLININT2D)
        (ifirstc(0), ifirstc(1),
    ...
    else if (d_dim == tbox::Dimension(3))
    {
        ...
        double *tpresc = new double[mc];
        double *tvelc = new double[mc];
        SAMRAI_F77_FUNC(conservlinint3d, CONSERVLININT3D)
        (ifirstc(0), ifirstc(1),
         ifirstc(2), /* input */
    ...
    const hier::IntVector &ratio)
{
    ...
    if (d_dim == tbox::Dimension(2))
    {
        SAMRAI_F77_FUNC(conservavg2d, CONSERVAVG2D)
        (ifirstf(0), ifirstf(1), ilastf(0),
         ilastf(1), /* input */
    ...
    {
        SAMRAI_F77_FUNC(conservavg3d, CONSERVAVG3D)
        (ifirstf(0), ifirstf(1), ifirstf(2), /* input */
         ilastf(0), ilastf(1), ilastf(2),
        ...
        if (time_allowed)
        {
            ...
            if (ref == "DENSITY_RICHARDSON" || ref == "PRESSURE_RICHARDSON")
            {
                     ...
                     ic != icend; ++ic)
                {
                     ...
                     * Compute error norm
                     */
                    diff = (*advanced_coarse_var)(*ic, 0) - (*coarsened_fine_var)(*ic, 0);
                    error =
    ...
    if (variable_name == "Total Energy")
    {
            ...
            for (int i2 = 0; i2 < box_w2; ++i2)
            {
                ...
                for (int i1 = 0; i1 < box_w1; ++i1)
                {
                    for (int i0 = 0; i0 < box_w0; ++i0)
                    {
                        ...
                        double v2norm = pow(xvel[dat_indx], 2.0) + pow(yvel[dat_indx], 2.0) + pow(zvel[dat_indx], 2.0);
        ...
        if (d_dim == tbox::Dimension(2))
        {
            ...
            for (int i1 = 0; i1 < box_w1; ++i1)
            {
                ...
                {
                    size_t dat_indx = dat_b1 + i0;
                    double v2norm = pow(xvel[dat_indx], 2.0) + pow(yvel[dat_indx], 2.0);
                    double rho = dens[dat_indx];
                    double int_energy = 0.0;
                    if (rho > 0.0)
                    {
    ...
    std::ostream &file)
{
    ...
    if (!box.empty())
    {
        ...
        const double cell_center = xlo[idir] + (double(box.lower(idir) - patchbox.lower(idir)) + 0.5) * dx[idir];
    ...
    bool is_from_restart)
{
    ...
    if (!is_from_restart)
    {
             ...
             (d_data_problem == "STEP")))
        {
            ...
            while (!found_interval_data && (i < d_number_of_intervals) && (nkey < static_cast<int>(init_data_keys.size())))
            {
