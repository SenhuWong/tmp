...
class Euler : public tbox::Serializable,
...
private:
    ...
    // Change::Add a tecplot writer
    std::shared_ptr<TecplotWriter> d_tecplot_writer;

    std::shared_ptr<pdat::CellVariable<int>> d_iblank;
    std::shared_ptr<hier::ComponentSelector> d_overset_flags;

public:
//Interp needs to get Iblank,primVars from Euler so that it could perform zip and perform pre and post process
//Overset needs only to register Variable at myEuler, and only needs a flag context from Euler2d

//tecplot writer should also be passed to its subStrategies.
//The only job remained is to find a correct timing for interpolation.
    void registerTecplotWriter(std::shared_ptr<TecplotWriter> tec_writer)
    {
        d_tecplot_writer = tec_writer;
    }
    std::shared_ptr<SAMRAI::pdat::CellVariable<int>> get_var_Iblank()
    {
        return d_iblank;
    }
    // It should also keep a list of Conservative Variable indexes at current(scratch) time
    // so that InterpStrategy could have access to the to_zip_data.
    //

    //%TODO::We require myHyperbolicLevelIntegrator's init function to specify d_interpContext;
    std::shared_ptr<hier::VariableContext> d_interpContext;
    void set_interp_context(std::shared_ptr<hier::VariableContext> interpContext)
    {
        d_interpContext = interpContext;
    }
    std::shared_ptr<hier::VariableContext> getInterpContext()
    {
        return d_interpContext;
    }
    //
    std::list<std::shared_ptr<hier::Variable>> d_prim_vars;
    std::list<std::shared_ptr<hier::Variable>> getBasicVariables()
    {
        return d_prim_vars;
    }

    //Overset stratey still do its job:
    //At initialization time tag boundaries.
    //At GradientDetector time perform tagGradientCell
    std::shared_ptr<OversetStrategy> d_oversetStrategy;
    //Interp Strategy still do its job:
    //form CartBlock.
    //Take CartBlock and CartGrid out.
    std::shared_ptr<InterpStrategy> d_interpStrategy;
    //Corresponding methods should be added to Euler2D

    //Function used to register overset operation related variables
    void registerOversetVariable(
    ...
    //Function to be called by applyGradientDetector(or other detector if called)
    void applyGeometricRefineDetector(
        ...
        const bool initial_time,
        const bool uses_richardson_extrapolation_too);
    //Function to be called to get CartBlock and CartGrid.
    ...
    //What else do we need to keep the above equation working?
    //1.Modify the initializeLevel at HyperbolicLevelIntegrator
    //(or could add the OversetStrategy to the initializeDataOnPatch)
};
